...
La generación opcional de SQL en Administración usa `POST /api/backend/sql/generate` como lectura autenticada y queda fuera del journal de mutaciones. `OPENAI_API_KEY` y `OPENAI_SQL_MODEL` se leen exclusivamente en backend; el navegador nunca recibe credenciales. Para habilitarla, se define `OPENAI_API_KEY` en el `.env` local ignorado por Git o como variable de entorno del proceso y se reinicia el servidor con `npm.cmd run server:restart`; nunca se guarda la clave en el frontend, el Editor de datos o archivos versionados. El proveedor recibe únicamente el pedido acotado del usuario y metadatos sanitizados del esquema visible (tablas, columnas, tipos y relaciones), sin filas, importes, personas, resultados SQL ni archivos. La propuesta se valida con la política read-only compartida, se compila contra tablas vacías construidas desde ese esquema y se envuelve con un límite exterior de 500 filas antes de devolverla al editor sin ejecutarla. El backend admite una sola generación simultánea por identidad, aplica un límite temporal por usuario y acota la concurrencia global; también corta respuestas del proveedor que superan 50 KB. Configuración ausente, autenticación 401/403, cuota o rate limit 429, 5xx del proveedor, timeout, DNS/TLS/red, respuesta malformada y SQL incompatible se clasifican por separado con cuerpo sanitizado; ante cualquier falla, la consola SQL manual y su contenido permanecen intactos.
