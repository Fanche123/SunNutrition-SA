...
Cada pedido o corrección explícita tiene una única validación principal a cargo del hilo o subagente ejecutor, con criterios de completitud, pruebas proporcionales y evidencia verificable. El Coordinador no repite controles rutinarios: revisa el resumen para detectar cobertura faltante, inconsistencias, riesgos o decisiones. `validador_tarea`, configurado con `gpt-5.6-sol`, esfuerzo high y sandbox read-only, se conserva únicamente para una revisión independiente excepcional y focalizada cuando exista alto riesgo, señales de fallo, evidencia insuficiente o alcance transversal sensible. El hilo debe declarar la razón, transmitir taskId, diff propio y cambios preexistentes delimitados; un árbol sucio conocido no bloquea salvo superposición material.
