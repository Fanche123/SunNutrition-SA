...
- Cashflow: posición canónica read-only de Gestión de Tesorería para bancos, `caja_efectivo_movimientos` y cheques; `egresos`, `detalle_pagos`, `ventas`, `cobros_detalle`, `clientes`, `acreedores` y fuentes para resolver contraparte.
...
- Cashflow: `cashflow.service.js`; Bancos, Efectivo, Cheques y Cheques a cubrir reutilizan `buildTreasuryManagementSnapshot` de Tesorería. Por cobrar y Deudas conservan saldo = total menos aplicaciones; el filtro temporal de eventos no reduce las tarjetas canónicas de posición de Tesorería. Si falta Caja Efectivo, tarjeta y saldos semanales quedan no disponibles sin presumir cero.
