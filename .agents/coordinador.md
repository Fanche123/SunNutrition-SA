...
>
> Cuando el impacto visual requiera inspección de una pantalla o interacción web normal, preferí `browser:control-in-app-browser`. No uses `chrome:control-chrome` de rutina: solo puede pedirse por una sesión autenticada exclusiva del perfil Chrome, una extensión instalada allí —incluido ARCA que dependa explícitamente de ella— o una solicitud expresa del usuario, y el hilo debe declarar antes esa dependencia concreta. Si no requiere UI, preferí pruebas focalizadas, HTTP/API o inspección DOM; si el navegador integrado no está disponible, usá primero esas alternativas seguras y aisladas. No abras ambos navegadores ni repitas la misma validación visual.
>
