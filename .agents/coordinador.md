...
>
> Antes de responder, aplicá íntegramente el “Gate operativo obligatorio de cierre” de `AGENTS.md`: dejá el servidor principal Local de `C:\Users\benja\Documents\ERP` activo en `127.0.0.1:3000`, exigí `running_fresh` y `ocr_reachable`, limpiá procesos temporales propios e informá PID, host/puerto, checkout/cwd y ambos estados. `running_fresh` solo no permite cerrar. Si no podés restaurar ambos estados con acceso normal de red, quedá `blocked`. En Worktree usá puerto aislado, timeout y cleanup, no reinicies el Local ni sirvas el Worktree en 3000; verificá al final que el Local sigue operativo. El ejecutor es responsable: ningún control read-only reinicia servidores. En riesgo alto ejecutá el gate después de liberar los tres validadores y antes del consolidador, que solo revisa la evidencia; repetilo si una corrección posterior altera el runtime.
